...
(function () {
  ...
  if (navigator.userAgent.match(/Trident|MSIE/)) {
    location = 'microsoft-edge:https://www.google.com';
    setTimeout(function () {
...
function gradient(gd, stops) {
  ...
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      var objColor = _step.value;

      var _Object$entries = Object.entries(objColor),
          _Object$entries2 = _slicedToArray(_Object$entries, 1),
          stop = _Object$entries2[0];

      var _stop = _slicedToArray(stop, 2),
          color = _stop[0],
          offset = _stop[1];
...
(function () {
  ...
  function initPianoCanvas() {
    ...
    var pianoPrev = piano.previousElementSibling;
    var pianoPrevHeight = +getComputedStyle(pianoPrev).height.split('px')[0];
    var pianoParent = piano.parentElement;
  ...
  function createKeyboard() {
    var ratio = 1.5;
    var whiteWidth = 22 * ratio;
    var height = 147 * ratio;
    var top = piano.height - height;
    var blackWidth = 11 * ratio;
    var allKeys = 21;
    var offsetX = (piano.width - allKeys * whiteWidth) / 2;
    c.textAlign = 'center';

    for (var i = 0; i < allKeys; i++) {
      var x = whiteWidth * i + 1;
      var octave = (i / 7 | 0) + 4 + '';
      var note = String.fromCharCode(65 + (i + 2) % 7) + octave;

      if (!(i % 1)) {
        var Hz = frquency(i + 40);
  ...
  function frquency(n) {
    var fixed = Math.pow(10, 4);
    return (Math.pow(2, (n - 49) / 12) * 440 * fixed | 0) / fixed;
  }

  var tabBoook = document.getElementById('list-book-list');
  var c;
  var keyboard;
  ...
  function keydownHandler(e) {
    ...
    var musicalNote = keyPairs[e.key.toLocaleLowerCase()];
    var activate = keyboard.filter(function (key) {
  ...
  tabBoook.addEventListener('shown.bs.tab', function (e) {
    var activateKey;
    c = initPianoCanvas();
    keyboard = createKeyboard().sort(function (a, b) {
      if (a.type > b.type) {
        return -1;
      }
    ...
    piano.addEventListener('mousedown', function (e) {
      var trigX = e.x - piano.offsetLeft;
      var trigY = e.y - piano.offsetTop;
      activateKey = keyboard.filter(function (key) {
        var matchX = isBetween(trigX, [key.left, key.right]);
        var matchY = isBetween(trigY, [key.top, key.bottom]);
  ...
  function init(samurai, _ref) {
    ...
    for (var y = 0; y < height; y++) {
      ...
      for (var x = 0; x < width; x++) {
        ...
        var alpha = data[colorSet + 3] / 255 * 0.8;
        var p1 = {
          x: 300 + x,
          y: finalImg.y + y + 3
        };
        var p0 = {
          x: p1.x + between(300, -300),
          y: innerHeight + between(100)
        };
...
var PianoKey = /*#__PURE__*/function () {
  ...
  _createClass(PianoKey, [{
    ...
    value: function draw(c) {
      ...
      if (type === 'black') {
        var topGd = c.createLinearGradient(left, top, left, bottom);
        var btmGd = c.createLinearGradient(left, top, right, bottom);
    ...
    key: "keydownBlackKey",
    value: function keydownBlackKey(c, topGd, btmGd) {
      var left = this.left,
          top = this.top,
          width = this.width,
          height = this.height;
    ...
    key: "keyupBlackKey",
    value: function keyupBlackKey(c, topGd, btmGd) {
      var left = this.left,
          top = this.top,
          width = this.width,
          height = this.height;
...
var Particle = /*#__PURE__*/function () {
  function Particle(args) {
    ...
    var def = {
      alpha0: 0,
      size: 5,
      duration: 500 + Math.random() * 2000 | 0
    };
    Object.assign(def, args);
    Object.assign(this, def);
